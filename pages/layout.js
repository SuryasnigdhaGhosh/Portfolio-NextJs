  ...
  return (
    <div className={`h-screen lg:p-[0.8rem] flex flex-col select-none font-circular`}>
      <div className='lg:hidden'>
        <div className=' bg-DeepNightBlack text-LightGray w-full h-10 flex items-center justify-between px-2 lg:hidden relative'>
          <div className='icon flex items-center gap-x-2' onClick={(e) => setIntro(!intro)}>
            <span className='icon border-2 text-violet-900 border-violet-700 p-1 text-sm rounded-lg'>
              <SlOptionsVertical />
            </span>
          ...
          <div className='icon flex items-center gap-x-2' onClick={(e) => setIsOpen(!isOpen)}>
            <span className='icon border-2 text-violet-700 border-violet-700 p-1 text-sm rounded-lg'>
              {' '}
              <FaBars />
            </span>
          </div>
      ...
      <div className='flex relative h-full justify-between gap-x-3'>
        {/* left most side */}
        <div
          className={`w-64 h-screen left-0 lg:rounded-xl -top-10 lg:top-0 transition lg:left-0 lg:h-full overflow-hidden bg-DeepNightBlack shadow-2xl z-50 lg:flex flex-col  lg:relative ${
        ...
        {/* right side */}
        {/* right side */}
          <div onClick={(e) => setIsOpen(!isOpen)} className=' text-LightViolet hidden lg:flex items-center h-16 justify-center text-2xl cursor-pointer '>
            <span className='icon border-2 border-LightViolet p-2 rounded-xl'>
              {' '}
              <FaBars />
            </span>
          </div>
